...
The narrow interception point is immediately after `SelectionCommand#call` creates the `/set` operation and before Enhanced's queue shortcut or `Operations.completeBlindly`. The bridge accepts only the exact `RegionVisitor -> BlockReplace -> same EditSession` shape and constant block already accepted by Stage 4. Success records the affected count on the visitor and jumps only over traversal to the existing status-message code; `null` falls through at the original instruction, before any Overdrive mutation. The legacy `EditSession#setBlocks` hook remains separately diagnosed but no longer determines ACTIVE status.

## Mechanism and fallback

Forge 1.7.10 does not provide a bundled, dependable Mixin service. Stage 4 uses a launch-time, method-descriptor-and-anchor-checked `IClassTransformer` as the 1.7-compatible equivalent of a one-method redirect. It inserts a nullable call to `Stage4SetBridge.trySetOperation`; `null` continues into the untouched shortcut/traversal path, while success skips traversal and rejoins existing feedback. A descriptor or semantic-anchor mismatch fails startup rather than patching an unknown Enhanced version. No WorldEdit-owned class is packaged.
