...
@Mod(modid = WorldEditOverdrive.MOD_ID, name = WorldEditOverdrive.MOD_NAME,
     version = WorldEditOverdrive.VERSION,
     dependencies = "required-after:worldedit", acceptableRemoteVersions = "*")
public final class WorldEditOverdrive {
  ...
  private final OverdriveConfiguration configuration =
      OverdriveConfiguration.defaults();
  private final OverdriveTickHandler ticks = new OverdriveTickHandler();

  @Mod.EventHandler
  public void initialize(FMLInitializationEvent event) {
    ModContainer worldEdit =
        Loader.instance().getIndexedModList().get("worldedit");
    String fmlVersion =
        worldEdit == null ? "not present" : worldEdit.getVersion();
    ...
    if (!sameVersion(apiVersion, fmlVersion))
      OverdriveLog.info("version diagnostics: FML={}, WorldEdit API={}",
                        fmlVersion, apiVersion);
    String reason = Stage4HookStatus.activeSetCommandHookInstalled ? ""
                    : Stage4HookStatus.selectionCommandSeen
                        ? " (SelectionCommand descriptor/anchors did not match)"
                        : " (SelectionCommand target not transformed)";
    OverdriveLog.info(
        "WorldEdit {} detected; Stage 4 //set hook {}{}", fmlVersion,
        Stage4HookStatus.activeSetCommandHookInstalled ? "ACTIVE" : "INACTIVE",
        reason);
    FMLCommonHandler.instance().bus().register(ticks);
  }

  private static boolean sameVersion(String api, String fml) {
    return api != null && fml != null &&
        !api.toLowerCase().contains("unknown") && api.equals(fml);
  }

  @Mod.EventHandler
  public void serverStarting(FMLServerStartingEvent event) {
    event.registerServerCommand(new OverdriveCommand(this));
  }

  @Mod.EventHandler
  public void serverStarted(FMLServerStartedEvent event) {
    coordinator = new OverdriveCoordinator(configuration);
    com.glowingfederal.worldeditoverdrive.integration.DeferredPasteManager
  ...
  /**
   * Internal Stage 3 API; command/session integration intentionally does not
   * use it yet.
   */
