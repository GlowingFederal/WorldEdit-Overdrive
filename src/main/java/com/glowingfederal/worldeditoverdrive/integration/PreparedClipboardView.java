...
public final class PreparedClipboardView {
    ...
    private final short[] states; private final Map<Integer,CompoundTag> tiles;
    public PreparedClipboardView(int minX,int minY,int minZ,int sizeX,int sizeY,int sizeZ,int originX,int originY,int originZ,
            short[] states,Map<Integer,CompoundTag> tiles) {
    ...
    public int packedStateAt(int x,int y,int z){return states[index(x,y,z)]&0xffff;}
    ...
    public long estimatedBytes(){return 36L+states.length*2L+tiles.size()*48L;}
}
