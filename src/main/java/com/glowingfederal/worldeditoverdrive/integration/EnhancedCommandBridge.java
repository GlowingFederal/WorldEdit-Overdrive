...
public final class EnhancedCommandBridge {
    ...
    public static Integer replace(EditSession session,Region region,Mask mask,Pattern pattern)throws MaxChangedBlocksException{
        ...
        complete("replace",wall,CommandHookStatus.replaceAccelerated);return Integer.valueOf(changed);
    }
    public static Integer geometry(EditSession session,Region region,Pattern pattern,int kind)throws MaxChangedBlocksException{
        ...
        int changed=apply(session,out,pattern);complete(kind==0?"walls":kind==1?"faces":"center",wall,CommandHookStatus.geometryAccelerated);return Integer.valueOf(changed);
    }
    public static Integer overlay(EditSession session,Region region,Pattern pattern)throws MaxChangedBlocksException{
        ...
        CommandHookStatus.lastOperationSnapshotMillis.set(ms(snap));int changed=apply(session,out,pattern);complete("overlay",wall,CommandHookStatus.overlayAccelerated);return Integer.valueOf(changed);
    }
    public static Integer naturalize(EditSession session,Region region)throws MaxChangedBlocksException{
        ...
        complete("naturalize",wall,CommandHookStatus.naturalizeAccelerated);return Integer.valueOf(changed);
    }
    public static Integer stack(EditSession session,Region region,Vector dir,int count,boolean copyAir)throws MaxChangedBlocksException{
        ...
        for(int n=1;n<=count;n++)for(int i=0;i<src.size();i++)if(session.setBlock(src.get(i).add(step.multiply(n)),blocks.get(i)))changed++;CommandHookStatus.lastOperationCommitMillis.set(ms(commit));complete("stack",wall,CommandHookStatus.stackAccelerated);return Integer.valueOf(changed);
    }
    public static Integer move(EditSession session,Region region,Vector dir,int distance,boolean copyAir,BaseBlock leave)throws MaxChangedBlocksException{
        ...
        for(Vector p:src)if(session.setBlock(p,replacement))changed++;for(int i=0;i<src.size();i++)if(session.setBlock(src.get(i).add(delta),blocks.get(i)))changed++;CommandHookStatus.lastOperationCommitMillis.set(ms(commit));complete("move",wall,CommandHookStatus.moveAccelerated);return Integer.valueOf(changed);
    }
